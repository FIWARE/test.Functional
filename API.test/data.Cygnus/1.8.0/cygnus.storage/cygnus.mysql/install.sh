...
cp target/cygnus-common-1.8.0-jar-with-dependencies.jar $FLUME_HOME/plugins.d/cygnus/libext/ 
$MVN_HOME/bin/mvn install:install-file -Dfile=$FLUME_HOME/plugins.d/cygnus/libext/cygnus-common-1.8.0-jar-with-dependencies.jar -DgroupId=com.telefonica.iot -DartifactId=cygnus-common -Dversion=1.8.0 -Dpackaging=jar -DgeneratePom=false 

# Cygnus-ngsi
echo "Compile Cygnus-ngsi" 
cd $CYGNUS_HOME/cygnus-ngsi
${MVN_HOME}/bin/mvn clean compile exec:exec assembly:single
cp target/cygnus-ngsi-1.8.0-jar-with-dependencies.jar $FLUME_HOME/plugins.d/cygnus/lib/
...
cp agent_ngsi.conf.template agent_ngsi_mysql.conf
cp cygnus_instance.conf.template cygnus_instance_mysql.conf

echo "Please edit agent_ngsi_mysql.conf and cygnus_instance_mysql.conf files in "$FLUME_HOME/conf" folder according to your mysql db instance" 
